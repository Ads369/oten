def level_old(self, level_number=None):
    """
    ...
    if json:
        ...
        task_md = self.handler_task(ene.level.tasks[0].task_text_formatted)

        args = {
            'lvl_num': ene.level.number,
            'lvl_cnt': len(ene.levels),
            'lvl_name': ene.level.name,
            'timeout': ene.level.timeout_seconds_remain,
            'answer_block': ene.level.has_answer_block_rule,
            ...
            'task': ene.level.tasks[0].task_text_formatted,
            'task_md': task_md,
            'hint_counts': len(ene.level.helps),
            'bonus_count': len(ene.level.bonuses),
        }

        msg = 'Уровень {lvl_num} из {lvl_cnt}: {lvl_name}\n' \
              '⏳:{timeout}\n' \
              '------------------------------\n' \
              '{task_md}'.format(**args)
    else:
        log.debug('have not json')
...
def unit_test():
    oten = Oten()
    html = '<br>\r\n  <audio controls>\r\n    <source src=\"http://d1.endata.cx/data/games/71527/1_7843275943729587432543678321.mp3\" type=\"audio/mpeg\">\r\n  </audio>\r\n<br><br>\r\n<span style=color:yellow;><b>ФО:</b> последняя строчка</span>'
